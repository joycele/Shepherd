...
from SHESAgent import ConvexHullAgent
...
if __name__ == "__main__":
    ...
    convex_agent = ConvexHullAgent()

    # load q network agent
    ...
    for episode in range(NUM_OF_EPISODE):
        ...
        executeAction(agent_host, 5)
        convex_agent.clear_stack()
        while world_state.is_mission_running:
            time.sleep(0.1)
            world_state = agent_host.getWorldState()

            
            convex_agent.updateWorldState(world_state)
            if len(convex_agent.obj_stack) == 0:
                convex_agent.genConvexHull()
            vec = convex_agent.act()
            vel = 0.25
            position = (convex_agent.agent_pos[0] + vec[0] * vel, convex_agent.agent_pos[1] + vec[1] * vel)
            cmd = "tp {} 207 {}".format(position[0], position[1])
